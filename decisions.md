...
## 2026-08-26 — Language: one live value, and a write path that doesn't widen the tenancy boundary
**Decision:** `i18n.language` is the single source of truth for "this nurse's current language" at runtime. Nothing else caches it: no `preferredLanguage` prop, no copy on the nurse profile object, no component state. `nurses.preferred_language` is the durable record, read once per authenticated load and written on change, never rendered from. The three Claude helpers that vary by language (`parseVoiceToTask`, `generateHandoffSummary`, `generateSuggestions`) no longer take a `language` argument at all, they read the live value themselves, so no call site is able to pass a stale one.
**Why:** Four bug reports from testing looked like four bugs and were mostly two. Static UI read `i18n.language` while AI calls and the Web Speech instances read a separately-passed copy derived from the nurse profile. The copy happened to be correct, but the shape invited drift, and it made the reports impossible to tell apart from a genuine staleness bug without tracing every call site.
**Decision:** A nurse changes their own language through `set_my_language`, a SECURITY DEFINER function (`0007_nurse_language_rpc.sql`), not a table UPDATE. `nurses` still grants clients no UPDATE.
**Why:** The toggle could never persist, `0001` grants `select, insert` on `nurses` and there is no UPDATE policy, so the choice was lost at every sign-in. The obvious fix, `grant update` plus a `using (id = auth.uid())` policy, would also let a nurse rewrite their own `facility_id`, self-assign into another facility and read its patients. Postgres row policies cannot restrict which columns are written; a one-column function can. Approved before implementation, per `CLAUDE.md`'s rule on RLS changes.
**Also this session:** The sign-up language choice now survives the page load that email confirmation inserts into the flow. It travels in `user_metadata`, and `App.jsx` applies it when no `nurses` row exists yet, so facility selection renders in the chosen language instead of reverting to English. Facility selection also carries its own picker, it is the last step of sign-up and was the only screen in the flow with no way to change language.
**Known, not fixed here:** The deployed `claude-proxy` was still version 1 (2026-08-25), with no language handling at all, which is why SBAR, voice-parsed descriptions, and suggestion text stayed English regardless of the toggle. Deploy is a separate manual step from committing, `supabase functions deploy claude-proxy`.
