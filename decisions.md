...
## 2026-08-26 — Data-entry notice finalized, both languages, implementation starting
...
**Not yet done:** the actual in-app screen and acknowledgment gate. `0008_notice_acknowledgment.sql` adds tracking for this, implementation prompt to follow.
...
## 2026-08-27 — Voice patient capture: root cause was an undeployed Edge Function, not extraction logic
**What happened:** Patient voice capture was investigated and repeatedly "fixed" across several sessions, regex ordering bugs, claim-boundary overreach, diagnosis flexibility, French lead-ins, all of it hardening the local fallback parser. The actual cause of the unreliability was that `parsePatientFromVoice` had never been deployed to the Edge Function, so every capture silently ran the regex fallback instead of Claude. Resolved by deploying the function, no code change required.
**Why the fallback work wasn't wasted:** `CLAUDE.md`'s "fallback parser required for every prompt" rule means this code is still load-bearing, it's what runs if the Edge Function is ever unreachable, and the primary path itself falls back to it on an all-empty draft. Real resilience, just not the fix for this specific bug.
**Standing lesson:** added to `CLAUDE.md` as a rule, confirm which code path is live before changing extraction logic, next time this pattern shows up.

## 2026-08-27 — AI-suggested tasks and notes now actually persist
**Confirmed:** `handleSuggestionAddAsTask`'s local-only state bug, flagged in an earlier `FINAL_REVIEW.md` (`id: Date.now()`, never reaching Supabase), is fixed. Accepted suggestions now route through the same creation path as every other task and note, and survive reload.
